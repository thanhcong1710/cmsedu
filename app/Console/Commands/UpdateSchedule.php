...
class UpdateSchedule extends Command
{
    ...
    {
        $list_class = u::query("SELECT cl.* FROM classes AS cl LEFT JOIN sessions AS s ON s.class_id=cl.id WHERE cl.cls_iscancelled='no' AND s.class_day=3 AND product_id!=100");
       foreach($list_class AS $class){
        $id = u::generateRandomStringOrNumber(9,true);
        $schedule = new Schedule();
        $schedule->cjrn_id = $id;
        $schedule->cls_id = 0;
        $schedule->cjrn_classdate = '2025-12-31';
