...
//            ProcessExcel::styleCells($spreadsheet, "B$x", "FFFFFF", "black", 9, 0, 3, "left", "center", false);
    ...
    public function export02b(Request $request)
    {
        if ($session = $request->users_data) {
            ...
            $sheet->setCellValue('D4', 'Mã kế toán');
            $sheet->setCellValue('E4', 'Học sinh');
            $sheet->setCellValue('F4', 'Địa chỉ');
            $sheet->setCellValue('G4', 'Sản phẩm');
            $sheet->setCellValue('H4', 'Mã nhân viên');
            $sheet->setCellValue('I4', 'Tên nhân viên');
            $sheet->setCellValue('J4', 'Tên gói phí');
            $sheet->setCellValue('K4', 'Số buổi');
            $sheet->setCellValue('L4', 'Ngày dự kiến bắt đầu');
            $sheet->setCellValue('M4', 'Ngày dự kiến kết thúc');
            $sheet->setCellValue('N4', 'Loại bảo lưu');
            ...
            for ($i = 0; $i < count($branches); $i++) {
                $x = $i + 5;
                $student_address = $branches[$i]->address . ( $branches[$i]->district_name ? ', ' . $branches[$i]->district_name : '' ) . ( $branches[$i]->province_name ? ', ' . $branches[$i]->province_name : '' );
                $sheet->setCellValue('A' . $x, $i + 1);
                $sheet->setCellValue('B' . $x, $branches[$i]->branch_name);
                $sheet->setCellValue('C' . $x, $branches[$i]->crm_id);
                $sheet->setCellValue('D' . $x, $branches[$i]->accounting_id);
                $sheet->setCellValue('E' . $x, $branches[$i]->student_name);
                $sheet->setCellValue('F' . $x, $student_address);
                $sheet->setCellValue('G' . $x, $branches[$i]->product_name);
                $sheet->setCellValue('H' . $x, $branches[$i]->hrm_id);
                $sheet->setCellValue('I' . $x, $branches[$i]->cm_name);
                $sheet->setCellValue('J' . $x, $branches[$i]->tuition_fee_name);
                $sheet->setCellValue('K' . $x, $branches[$i]->summary_sessions);
                $sheet->setCellValue('L' . $x, $branches[$i]->start_date);
                $sheet->setCellValue('M' . $x, $branches[$i]->end_date);
                $sheet->setCellValue('N' . $x, $branches[$i]->is_reserved ? "Không giữ chỗ" : "Giữ chỗ");
                $sheet->getRowDimension($x)->setRowHeight(23);
                ProcessExcel::styleCells($spreadsheet, "A$x", 'FFFFFF', '111111', 11, 0, 3, "right", "center", true, 0, 'Cambria');
                ProcessExcel::styleCells($spreadsheet, "B$x", 'FFFFFF', '111111', 11, 0, 3, "left", "center", true, 0, 'Cambria');
                ProcessExcel::styleCells($spreadsheet, "C$x", 'FFFFFF', '111111', 11, 0, 3, "center", "center", true, 0, 'Cambria');
    ...
    public function exportLgl02(Request $request)
    {
        if ($session = $request->users_data) {
            ...
            $sheet->mergeCells('A4:H4');
            $sheet->setCellValue('A1', 'CÔNG TY CỔ PHẦN GIÁO DỤC LOGIC LAB');
            $sheet->setCellValue('A2', 'BÁO CÁO SỬ DỤNG VOUCHER');
            $sheet->getRowDimension('1')->setRowHeight(36);
            $sheet->getRowDimension('2')->setRowHeight(50);
            $sheet->getRowDimension('3')->setRowHeight(20);
            $sheet->getRowDimension('4')->setRowHeight(20);
            ...
            $sheet->setCellValue('A5', 'STT');
            $sheet->setCellValue('B5', 'Mã voucher');
            $sheet->setCellValue('C5', 'Trung tâm');
            $sheet->setCellValue('D5', 'Mã HS');
            $sheet->setCellValue('E5', 'Học sinh');
            $sheet->setCellValue('F5', 'Phụ huynh');
            $sheet->setCellValue('G5', 'SĐT');
            $sheet->setCellValue('H5', 'Người nhập');
            $sheet->setCellValue('I5', 'Thời gian nhập');
            $sheet->setCellValue('J5', 'Khóa học');
            ...
            for ($i = 0; $i < count($students); $i++) {
                ...
                $sheet->setCellValue('C' . $x, $students[$i]->branch_name);
                $sheet->setCellValue('D' . $x, $students[$i]->crm_id);
                $sheet->setCellValue('E' . $x, $students[$i]->name);
                $sheet->setCellValue('F' . $x, $students[$i]->gud_name1);
                $sheet->setCellValue('G' . $x, $students[$i]->gud_mobile1);
                $sheet->setCellValue('H' . $x, $students[$i]->creator_name);
                $sheet->setCellValue('I' . $x, $students[$i]->created_at);
                $sheet->setCellValue('J' . $x, $students[$i]->product_name);
                $sheet->setCellValue('K' . $x, $students[$i]->tuition_fee_name);
                $sheet->setCellValue('L' . $x, $students[$i]->must_charge);
                $sheet->setCellValue('M' . $x, $students[$i]->debt_amount);
                $sheet->getRowDimension($x)->setRowHeight(23);
                ProcessExcel::styleCells($spreadsheet, "A$x", 'FFFFFF', '111111', 11, 0, 3, "right", "center", true, 0, 'Cambria');
                ProcessExcel::styleCells($spreadsheet, "B$x", 'FFFFFF', '111111', 11, 0, 3, "left", "center", true, 0, 'Cambria');
                ProcessExcel::styleCells($spreadsheet, "C$x", 'FFFFFF', '111111', 11, 0, 3, "center", "center", true, 0, 'Cambria');
    ...
    public function exportLgl04(Request $request)
    {
        if ($session = $request->users_data) {
            ...
            $sheet->setCellValue('A5', 'STT');
            $sheet->setCellValue('B5', 'Trung tâm');
            $sheet->setCellValue('C5', 'Tên HS');
            $sheet->setCellValue('D5', 'Mã HS');
            $sheet->setCellValue('E5', 'Mã chiết khấu');
            $sheet->setCellValue('F5', 'Hình thức bán');
            $sheet->setCellValue('G5', 'Người nhập');
            $sheet->setCellValue('H5', 'Thời gian nhập');
            $sheet->setCellValue('I5', 'Gói phí');
    ...
    public function exportLgl03(Request $request)
    {
        if ($session = $request->users_data) {
            ...
            $sheet->setCellValue('A1', 'CÔNG TY CỔ PHẦN GIÁO DỤC LOGIC LAB');
            $sheet->setCellValue('A2', 'BÁO CÁO TỔNG HỢP GÓI BÁN');
            $sheet->getRowDimension('1')->setRowHeight(36);
            $sheet->getRowDimension('2')->setRowHeight(50);
            $sheet->getRowDimension('3')->setRowHeight(20);
            $sheet->getRowDimension('4')->setRowHeight(20);
            ...
            $sheet->setCellValue('A5', 'STT');
            $sheet->setCellValue('B5', 'Trung tâm');
            $sheet->setCellValue('C5', 'Mã chiết khấu');
            ...
            ProcessExcel::styleCells($spreadsheet, "A4:F4", NULL, NULL, 12, 1, 3, "center", "center", true, 0, 'Arial');
