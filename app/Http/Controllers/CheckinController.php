...
class CheckinController extends Controller
{
    ...
    {
        $sql = "SELECT *,
                (select ec_id from `term_student_user` where student_id = $id LIMIT 1) ec_id,
                (select cm_id from `term_student_user` where student_id = $id  LIMIT 1) cm_id,
                (select full_name from users where id = ec_id  LIMIT 1) as ec_name,
                (select name from branches where id = students.branch_id  LIMIT 1) as branch_name,
                (select name from `shifts` where id = students.shift_id  LIMIT 1) as shift_name,
                (select concat(full_name,' - ',hrm_id) from users where id = students.creator_id  LIMIT 1) as creator_name,
                (SELECT t.role_id FROM `term_user_branch` t WHERE t.`user_id` = students.creator_id ORDER BY id DESC LIMIT 1) as role_id,
                (select s1.crm_id from students s1 where s1.id = students.sibling_id  LIMIT 1) as sibling_crm_id,
                (select s1.name from students s1 where s1.id = students.sibling_id  LIMIT 1) as sibling_name,
                (select full_name from users where id = cm_id  LIMIT 1) as cm_name,
