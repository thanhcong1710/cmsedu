<template>
	<modal name="tool-bar1" @before-close="beforeClose" width="90%" height="96%" :draggable="true" :adaptive="true" :scrollable="true" :resizable="true" classes="tools-bar">
      <div class="tool-bar-frame modal-primary ada-modal">
        <header class="modal-header">
          <h5 class="modal-title">Công cụ hỗ trợ - Tính ngày học cuối</h5>
          <button @click="$modal.hide('tool-bar1')" class="close">×</button>
        </header>
        ...
        <footer class="modal-footer">
          ...
          <button type="button" @click="$modal.hide('tool-bar1')" class="ada-btn danger">Close</button>
        </footer>
      </div>
    </modal>
</template>
...
import datepicker from 'vue2-datepicker'
  ...
  methods: {
    ...
    calculateLastDate() {
      if (this.start_date && this.classdays.length && this.holidates.length) {
        const start_date = this.moment(this.start_date).format('YYYY-MM-DD')
        const info = u.calEndDate(this.sessions, this.classdays, this.holidates, start_date)
