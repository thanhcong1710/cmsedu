<template>
  ...
  >
    <div class="col-12">
      <b-card header>
        <div slot="header">
          <i class="fa fa-filter" /> <b class="uppercase">Form Import Payment</b>
        </div>
        <div
          id="filter_content"
          class="apax-form"
                  ...
                  <div class="table-responsive scrollable">
                    ...
                    >
                      <thead>
                        <tr>
                          <th>STT</th>
                          <th>Mã hợp đồng (số chứng từ nhập học)</th>
                          <th>Mã gói phí</th>
                          <th>Mã phiếu thu</th>
                          <th>Mã nhân viên thu ngân</th>
                          <th>Số tiền thu</th>
                          <th>Ngày thu</th>
                          <th>Ghi chú</th>
                          <th>Trạng thái</th>
                      ...
                      <tbody>
                        ...
                        >
                          <td>{{ student.stt }}</td>
                          <td>{{ student.contract_code }}</td>
                          <td>{{ student.tuition_code }}</td>
                          <td>{{ student.payment_code }}</td>
                          <td>{{ student.creator }}</td>
                          <td>{{ student.amount }}</td>
                          <td>{{ student.charge_date }}</td>
                          <td>{{ student.note }}</td>
                          <td
...
export default {
  ...
  methods: {
    uploadFile () {
      ...
      if (!this.stage) {
        this.stage = true
        u.p('/api/upload/payment', formData).then((response) => {
    ...
    executeImportContract () {
      ...
      if (!this.stage) {
        this.stage = true
        u.p('/api/import/payment', formData).then((response) => {
          this.students      = response.data
          this.executeImport = response.success
          this.notify        = response.message
          this.pending       = false
    ...
    downloadTemplateContract () {
      window.open('/api/payment/download-template', '_blank')
    },
  },
}
</script>
