                    ...
                    >
                      <thead>
                        <tr>
                          ...
                          <th>Quận / Huyện</th>
                          <th>Phụ huynh 1</th>
                          <th>Số điện thoại phụ hunh 1</th>
                          <th>Email phụ huynh 1</th>
                          <th>Trạng thái</th>
                        </tr>
                      </thead>
                      <tbody>
                        ...
                        >
                          ...
                          <td>{{ student.ec_code }}</td>
                          <td>{{ student.ec_leader }}</td>
                          ...
                          <td>{{ student.parent1_name }}</td>
                          <td>{{ student.parent1_phone }}</td>
                          <td>{{ student.parent1_email }}</td>
                          <td
...
export default {
  ...
  methods: {
    uploadFile () {
      ...
      if (!this.stage) {
        this.stage = true
        u.p('/api/upload/students', formData).then((response) => {
    ...
    executeImportStudent () {
      const formData = new FormData()
      formData.append('file', this.file)
      this.pending   = true
      if (!this.stage) {
        this.stage = true
        u.p('/api/import/students', formData).then((response) => {
          this.students      = response.data
          this.executeImport = response.success
          this.notify        = response.message
          this.pending       = false
    ...
    downloadTemplateStudent () {
      window.open('/api/student/download-template', '_blank')
    },
  },
}
</script>
