<!--TODO Mới bổ xung thêm trường hợp chỉ view và download file, còn trường hợp link là ảnh thì chưa xử lý-->
<template>
	<div v-if="readonly" class="apax-file">
		<a target="_blank" :href="link" class="download-file">
      <label>
	...
	<div v-else v-b-tooltip.hover :title="title" class="apax-file upload form-group frame" :id="id" :class="full ? customClass : `mini ${customClass}`">
			...
			if (e.target.files.length && e.target.files[0]) {
				...
				fileReader.onload = (e) => {
					...
					if (validate) {
						...
						let newName = ` - ${_.get(file, 'label.length') > 9 ? u.sub(file.label, 9) : file.label}.${file.ext} (${fileSize}KB)`
						if (parseInt(this.short)) {
							newName = parseInt(this.short) < 9 ? ` - File.${file.ext}` : ` - ${u.sub(file.label, parseInt(this.short))}.${file.ext}`
						}
...
.apax-file .download-file {
	text-decoration: none;
}
.apax-file .download-file label {
	background: #ff4f4f;
	text-shadow: 0 1px 1px #111;
	box-shadow: none;
	padding: 3px 6px;
...
.apax-file .download-file label i {
	margin-right: 5px;
	margin-left: 5px;
}
.apax-file .input-file:focus + label,
