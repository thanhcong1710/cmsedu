  ...
  <div class="animated fadeIn apax-form" id="charg-add" :class="html.class.ready" @click="checkingOnClick">
    <div class="row">
      ...
      <div class="col-12">
        <b-card header>
          <div slot="header">
            <i class="fa fa-money"></i> <b class="suitcase">Thông tin đóng tiền</b>
          </div>
...
export default {
  ...
  watch: {
    charge_amount: function (val) {
      if (this.item.must_charge) {
        ...
        const suma = value.n + parseInt(this.item.total_charged)
        const debt = parseInt(this.item.must_charge) - parseInt(suma)
  ...
  methods: {
    ...
    saveForm() {
      const charge_num = u.fmc(this.charge_amount)
      if ((parseInt(charge_num.n) + parseInt(this.item.total_charged)) > parseInt(this.item.must_charge)) {
        const new_charge_num = u.fmc(u.rd(parseInt(this.item.must_charge)) - u.rd(parseInt(this.item.total_charged)))
        ...
        const total_charged = parseInt(charge_num.n,10) + parseInt(this.item.total_charged,10)
