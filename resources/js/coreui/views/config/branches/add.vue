      ...
      <b-row>
        <b-col cols="12">
          <b-card header-tag="header"
                  footer-tag="footer">
            <div slot="header">
              <i class="fa fa-clipboard"></i> <strong>Thêm mới danh mục đơn vị cơ sở</strong>
            ...
            <div class="col-sm-4">
              <div class="form-group">
               ...
               <input type="text" v-model="branch.accounting_id" class="form-control" :readonly="false">
              </div>
            </div>
            <div class="col-sm-4">
              <div class="form-group">
              ...
              <div class="panel-footer">
                <div class="row">
                  <div class="col-sm-12 col-sm-offset-3 text-right">
                    <button class="apax-btn full edit" type="submit" @click="addNewBranch"><i class="fa fa-save"></i> Lưu thay đổi</button>
    ...
    methods: {
        addNewBranch(){
          ...
          if(!branch.name|| branch.name == ''){
            ...
            this.storeNewBranch(branch)
          }
        },
        storeNewBranch(branch){
          const data = branch
          u.a().post(`/api/branches`, data).then(response => {
            if(response.data == 0){
              alert('Mã HRM, EFFECT hoặc LMS đã tồn tại' );
              return false
            }
            else{
              this.html.modal.message = "Thêm mới thành công đơn vị cơ sở !"
