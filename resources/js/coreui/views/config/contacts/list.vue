  ...
  <div class="wrapper">
    <div class="animated fadeIn apax-form">
      <b-card header-tag="header">
        <div slot="header">
          <i class="fa fa-diamond"></i> <b class="uppercase">Gói Học Phí</b>
        </div>
        <div class="panel">
          <div class="row">
                ...
                <div class="panel listing-frame">
                  <div class="row">
                    <div class="form-group list">
                      ...
                      <select class="form-control tight" v-model="product_id" @change="selectProduct(product_id)">
                  ...
                  <div class="row">
                    <div class="form-group list" v-show="tuitions.length">
                      <label class="control-label tight">Chọn gói phí</label>
                      <ul class="form-control tight" id="tuitions">
                        <li class="row the-first">
                          <span class="item-tuition-id col-md-3">ID</span>
                          <span class="item-tuition-name col-md-8">Tên Gói Phí</span>
                        </li>
                        <li class="row" :class="{'selected-tuition-fee' : check(tuition.id)}" v-for="(tuition, index) in tuitions" :key="index" @click="selectTuition(tuition.id)">
                          <span class="item-tuition-id col-md-3">{{ tuition.id }}</span>
                          <span class="item-tuition-name col-md-8" :class="{'text-danger' : check(tuition.id)}"><i class="fa fa-money"></i> {{tuition.name}}</span>
                        </li>
                      </ul>
                    </div>
                    <abt
                  ...
                  <div class="row" v-show="listSelectedBranchs.length">
                    <ul class="applied-branches">
                      ...
                      <li v-for="(branch, index) in listSelectedBranchs" :key="index" class="row" @click="removeBranch(branch.id)">
                        <span class="item-branch-id col-md-3">{{ branch.id }}</span>
                        <span class="item-branch-name col-md-8"><i class="fa fa-remove"></i> {{ branch.name }}</span>
                      </li>
                    </ul>
                  </div>
                </div>
            ...
            <div class="col-md-7 title-bold">
              <b-card header-tag="header">
                <div slot="header">
                  <i class="fa fa-table"></i> <b class="uppercase">Thông tin thiết lập gói phí</b>
                </div>
                  ...
                  <div class="row">
                    <div class="col-md-12">
                      <div class="row">
                        ...
                        <span class="col-md-8 fline"><input v-model="product.name" class="form-control" type="text" readonly/></span>
                      </div>
                      <div class="row">
                        <span class="col-md-4 tline title-bold txt-right">Tên gói phí:</span>
                        <span class="col-md-8 fline"><input v-model="tuition.name" class="form-control" type="text" /></span>
                      </div>
                      <div class="row">
                        <span class="col-md-4 tline title-bold txt-right">Số buổi học:</span>
                        <span class="col-md-8 fline"><input type="number" class="form-control" v-model="tuition.session"></span>
                      </div>
                      <div class="row">
                        <span class="col-md-4 tline title-bold txt-right">Giá niêm yết:</span>
                        <span class="col-md-8 fline"><input type="text" class="form-control" v-model="tuition.price" @change="formatPrice(tuition.price)"></span>
                      </div>
                      <div class="row">
                        <span class="col-md-4 tline title-bold txt-right">Giá thực thu:</span>
                        <span class="col-md-8 fline"><input type="text" class="form-control" v-model="tuition.receivable" @change="formatReceivable(tuition.receivable)"></span>
                      </div>
                      <div class="row">
                        <span class="col-md-4 tline title-bold txt-right">Chiết khấu:</span>
                        <span class="col-md-8 fline"><input type="text" v-model="tuition.discount" @change="formatDiscount(tuition.discount)" class="form-control" plasceholder="Nếu nhập giá trị nhỏ hơn 100 sẽ tính là % còn nếu lớn hơn 1000 thì sẽ tính là tiền"></span>
                      </div>
                      <div class="row">
                      ...
                      <div class="row">
                        ...
                        <span class="col-md-8 fline">
                          <select class="form-control" v-model="tuition.type">
                      ...
                      <div class="row">
                        ...
                        <span class="col-md-8 fline">
                          <select class="form-control" v-model="tuition.status">
                            <option value="">Chọn loại đóng phí</option>
                      ...
                      <div class="row">
                        <span class="col-md-4 tline title-bold txt-right">Tương ứng April</span>
                        <span class="col-md-8 fline">
                          <vue-select 
                            label="name"
                            multiple  
                            :options="april_tuitions" 
                            v-model="april_selected"
                            placeholder="Chọn gói phí April tương ứng"
                            :searchable="true"
                            :onChange="selectAprilTuition"
                            language="en-US">  
                          </vue-select>
                        </span>
                      </div>
                      <div class="row">
                        <span class="col-md-4 tline title-bold txt-right">Tương ứng CDI4.0</span>
                        <span class="col-md-8 fline">
                          <vue-select 
                            label="name"
                            multiple  
                            :options="cdi_tuitions" 
                            v-model="cdi_selected"
                            placeholder="Chọn gói phí CDI4.0 tương ứng"
                            :searchable="true"
                            :onChange="selectCDItuition"
                            language="en-US">  
                          </vue-select>
                        </span>
                      </div>
                      <div class="row">
                        <span class="col-md-4 tline title-bold txt-right">Tương ứng iGarten</span>
                        <span class="col-md-8 fline">
                          <vue-select 
                            label="name"
                            multiple  
                            :options="igarten_tuitions" 
                            v-model="igarten_selected"
                            placeholder="Chọn gói phí iGarten tương ứng"
                            :searchable="true"
                            :onChange="selectiGartenTuition"
                            language="en-US">  
                          </vue-select>
                        </span>
                      </div>
            ...
            <b-modal title="Chọn các trung tâm được áp dụng gói phí" class="modal-success" size="lg" v-model="show" @ok="saveBranchs" @cancel="cancelBranchs" ok-variant="success">
              <div class="table-responsive header-filter">
                <table class="table table-bordered apax-table" id="table_list">
                  ...
                  <tbody>
                    <tr class="filter">
                      <td class="checkbox-item filter-row"><input id="iCheck" @click="checkAll('#iCheck')" name="CheckAll" type="checkbox" /></td>
                      <td class="id-filter filter-row"><input type="text" placeholder="ID" @input="search('id', $event.target.value)" :v-model="id_filter" /></td>
                      <td class="lms-filter filter-row"><input type="text" placeholder="Mã LMS" @input="search('lms', $event.target.value)" :v-model="lms_filter" /></td>
                      <td class="name-filter filter-row"><input type="text" placeholder="Tên trung tâm" @input="search('name', $event.target.value)" :v-model="name_filter" /></td>
                      <td class="zone-filter filter-row">
                        <select @change="find" v-model="zone_filter">
                      ...
                      <td class="region-filter filter-row">
                        <select @change="find" v-model="region_filter">
                ...
                <table class="table table-bordered table-striped apax-table content-detail">
                  <tbody>
                    <tr v-for="(branch, index) in branches" :key="index">
                      <td class="checkbox-item" @click="marking(branch)"><input v-model="listTemporary" :value="branch" :id="'branch_'+branch.id" type="checkbox" readonly /></td>
                      <td class="id-filter" @click="marking(branch)">{{ branch.id }}</td>
                      <td class="lms-filter" @click="marking(branch)">{{ branch.lms }}</td>
                      <td class="name-filter" @click="marking(branch)">{{ branch.name }}</td>
                      <td class="zone-filter" @click="marking(branch)">{{ branch.zone }}</td>
                      <td class="region-filter" @click="marking(branch)">{{ branch.region }}</td>                      
                    </tr>
                  </tbody>
                </table>
              </div>
  ...
  export default {
    ...
    created() {
      ...
      u.g(`/api/settings/tuitions/config/default`).then(response => {
        ...
        this.listTemporary = []
        this.igarten_tuitions = response.igarten_tuitions
        this.april_tuitions = response.april_tuitions
        this.cdi_tuitions = response.cdi_tuitions
      })
    },
    methods: {
      reset(){
      ...
      checkStatus() {
        const checked = parseInt(this.tuition.status, 10) === 1 ? true : false
      ...
      selectTuition(id){
        ...
        if(id){
          ...
          u.g(`/api/tuitions/select/${id}/load/`).then(response =>{
            ...
            this.cdi_selected = response.cdi_selected
            this.april_selected = response.april_selected
            this.igarten_selected = response.igarten_selected
            setTimeout(()=>{
              this.checking()
            },100)
          })
      ...
      updateTuitions(){
        var t = []
        for (var i in this.listSelectedBranchs){
      ...
      saveTuitions(){
          ...
          if (!validate){
            ...
            p.cdi = this.cdi_selected
            p.april = this.april_selected
            p.igarten = this.igarten_selected
            p.name = this.tuition.name
            p.session = this.tuition.session
            p.product_id = this.product_id
            p.branch_id = t
      ...
      removeBranch(id) {
        var result = confirm("Bạn có chắc chắn là muốn xoá trung tâm này?");
        if (result) {
