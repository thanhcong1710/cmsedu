              ...
              <div class="panel-footer">
                <div class="row">
                  <div class="col-sm-12 col-sm-offset-3 text-right">
                    <button class="btn btn-success" type="submit" @click.prevent="addFeeType">Lưu</button>
...
<script>
  ...
  export default {
    ...
    methods: {
      addFeeType(){
        axios.post(`/api/feetypes`, this.feetype).then(response => {
          this.$router.push('/feetypes')
          // this.feetype.push(response.data.feetype);

        });
        console.log(`Add feetype ${this.name}`);
      },
    }
  }
</script>
