                ...
                <div class="row">
                  ...
                  <div class="col-sm-3">
                    <div class="form-group">
                      ...
                      <select class="form-control" v-model="scoring_guideline.status">
              ...
              <div class="panel-footer">
                <div class="row">
                  <div class="col-sm-12 col-sm-offset-3 text-right">
                    <button class="apax-btn full edit" type="submit" @click="updateScoringGuideline"><i class="fa fa-save"></i> Lưu</button>
...
<script>
  ...
  export default {
    ...
    created(){
      u.a().get(`/api/scoring_guidelines/`+this.$route.params.id).then(response => {
        this.scoring_guideline = response.data.data;
        console.log(this.scoring_guideline);
      })
    },
    methods: {
      updateScoringGuideline(){
        let data = {
          score: this.scoring_guideline.score,
          guideline: this.scoring_guideline.guideline,
          explanation: this.scoring_guideline.explanation,
          status: this.scoring_guideline.status
        }
        if(data.score==='' || data.status==='' || data.guideline===''){
          alert("Điểm số và trạng thái không để trống !")
          return false
        }else {
          this.saveUpdateScoringGuideline()
        }
      },
      exitModal(){
          this.$router.push('/scoring-guidelines')
      },
      saveUpdateScoringGuideline(){
        const data = {
          explanation: this.scoring_guideline.explanation,
          guideline: this.scoring_guideline.guideline,
          score: this.scoring_guideline.score,
          id: this.scoring_guideline.id,
          status: this.scoring_guideline.status
        }
        let uri = `/api/scoring_guidelines/edit`;
        axios.put(uri, data).then((response) => {
          // this.$router.push('/scoring-guidelines')
          this.html.modal.display = true
          this.html.modal.message = "Sửa Hướng Dẫn Điểm Số Thành Công"
        })
      }
    }
  }
