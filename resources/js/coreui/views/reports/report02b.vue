            ...
            <div class="row">
              ...
              <div class="col-md-3">
                <select
                  ...
                  class="form-control"
                  placeholder="Chọn loại bảo lưu"
                >
                  <option value="-1">Chọn loại  bảo lưu</option>
      ...
      <div class="col-12">
        <b-card header>
          <div slot="header">
            ...
            <strong>Báo cáo chi tiết học sinh bảo lưu - Tổng số: {{pagination.total}}</strong>
          </div>
            ...
            >
              ...
              <tbody>
                <tr v-for="(item, index) in dataReport" :key="index">
                  ...
                  <td>{{item.end_date}}</td>
                  <td>{{item.is_reserved ? "Không giữ chỗ":"Giữ chỗ"}}</td>
                </tr>
              </tbody>
            </table>
          </div>
...
export default {
  ...
  methods: {
    search(a) {
      ...
      const link = "/api/reports/form-02b"
      u.p(link, data, 1)
        .then(response => {
    ...
    exportExcel() {
      ...
      var urlApi = "/api/export/report02b"
      var tokenKey = u.token()
      u.g(urlApi, params, 1, 1)
        .then(response => {
          saveAs(response, "Báo cáo chi tiết học sinh Bảo Lưu.xlsx");
          this.processing = false;
        })
        .catch(e => {
          this.processing = false;
