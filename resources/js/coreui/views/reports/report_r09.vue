      ...
      <div class="col-12">
        <b-card header>
          <div slot="header">
            ...
            <strong>BÁO CÁO HỌC SINH CHECKIN - PHỤ HUYNH SHOWUP</strong>
          </div>
            ...
            >
              ...
              <tbody>
                <tr v-for="(item, index) in dataReport" :key="index">
                  ...
                  <td>{{ item.update_checked }}</td>
                  <td>{{ item.updator_checked }}</td>
                </tr>
              </tbody>
            </table>
          </div>
...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      ...
      var urlApi = "/api/export/report_r09"
      var tokenKey = u.token()
      u.g(urlApi, params, 1, 1)
        .then(response => {
          saveAs(response, "BÁO CÁO HỌC SINH CHECKIN - PHỤ HUYNH SHOWUP.xlsx");
          this.processing = false;
        })
        .catch(e => {
          this.processing = false;
