      ...
      <div class="col-12">
        <b-card header>
          <div slot="header">
            ...
            <strong>BÁO CÁO SỬ DỤNG VOUCHER - Tổng số: {{pagination.total}}</strong>
          </div>
...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      ...
      var urlApi = "/api/export/lgl02"
      var tokenKey = u.token()
      u.g(urlApi, params, 1, 1)
        .then(response => {
          saveAs(response, "Báo cáo sử dụng voucher.xlsx");
          this.processing = false;
        })
        .catch(e => {
          this.processing = false;
