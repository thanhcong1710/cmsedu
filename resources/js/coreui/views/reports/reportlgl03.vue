        ...
        <b-card header>
          ...
          <div class="content-detail">
            <div class="row">
              ...
              <div class="col-md-3">
                <select v-model="searchData.typeNew"  class="search-field form-control">
                  <option value="" >Chọn hình thức bán</option>
                  <option value="1">Bán mới</option>
                  <option value="2">Tái tục</option>
                </select>
              </div>
              <div class="col-md-3">
                <select v-model="searchData.typePrice" class="search-field form-control">
                  <option value="" >Chọn gói bán</option>
                  <option value="1">Gói lẻ</option>
                  <option value="2">Gói combo</option>
                </select>
              </div>
              <div class="col-md-3">
                <select v-model="searchData.typePayment" class="search-field form-control">
                  <option value="" >Chọn trạng thái thu phí</option>
                  <option value="1">Đặt cọc</option>
                  <option value="2">Full fee</option>
                </select>
              </div>
            </div>
          </div>
      ...
      <div class="col-12">
        <b-card header>
          <div slot="header">
            ...
            <strong>BÁO CÁO TỔNG HỢP GÓI BÁN - Tổng số: {{pagination.total}}</strong>
          </div>
            ...
            >
              ...
              <tbody>
                <tr v-for="(item, index) in dataReport" :key="index">
                  ...
                  <td>{{ item.discount_code }} ( {{ item.discount_name }})</td>
                  <td>{{ item.count_pay }}</td>
                  <td>{{item.total_pay | formatMoney}}</td>
                  <td>{{ (item.count_pay >0 ? item.total_pay/item.count_pay :  '' )| formatMoney}}</td>
                </tr>
              </tbody>
            </table>
          </div>
...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      ...
      var urlApi = "/api/export/lgl03"
      var tokenKey = u.token()
      u.g(urlApi, params, 1, 1)
        .then(response => {
          saveAs(response, "Báo cáo tổng hợp gói bán.xlsx");
          this.processing = false;
        })
        .catch(e => {
          this.processing = false;
    ...
    getParamsSearch() {
      ...
      const data = {
        ...
        typePrice: this.searchData.typePrice,
        typeNew: this.searchData.typeNew,
        typePayment: this.searchData.typePayment,
      };
      return data;
    },
    clearSearch() {
