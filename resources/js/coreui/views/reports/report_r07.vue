      ...
      <div class="col-12">
        <b-card header>
          <div slot="header">
            ...
            <strong>BÁO CÁO TÌNH HÌNH HỌC SINH UCREA - {{title_report}}</strong>
          </div>
            ...
            >
              ...
              <tbody>
                <tr v-for="(item, index) in dataReport" :key="index">
                  ...
                  <td>{{ item.score_week_2 }}</td>
                  <td>{{ item.score_week_3 }}</td>
                  <td>{{ item.score_week_4 }}</td>
                  <td>{{ item.score_week_last }}</td>
...
export default {
  ...
  created() {
    ...
    this.search();
     u.g(`/api/report_student/get_report_month`)
      .then(response => {
  ...
  methods: {
    ...
    exportExcel() {
      ...
      var urlApi = "/api/export/report_r07"
      var tokenKey = u.token()
      u.g(urlApi, params, 1, 1)
        .then(response => {
          saveAs(response, "BÁO CÁO TÌNH HÌNH HỌC SINH UCREA.xlsx");
          this.processing = false;
        })
        .catch(e => {
          this.processing = false;
    ...
    getAllInfo(branch_id) {
      ...
      getAllClass(teacher_id) {
        if(teacher_id){
          u.a().get(`/api/report_student/class_info/${teacher_id}?product_id=1`)
            .then(response => {
              if (response.data.code == 200) {
