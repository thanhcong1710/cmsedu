      ...
      <div class="col-12">
        <b-card header>
          <div slot="header">
            ...
            <strong>BÁO CÁO HỌC SINH MỚI</strong>
          </div>
            ...
            >
              <thead>
                <tr class="text-sm">
                  ...
                  <th>Đã đóng</th>
                  <th>Tư vấn viên</th>
                  <th>Tình trạng học sinh</th>
                  <th v-if="['999999999',102].indexOf(role_id)!= -1">SĐT</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(item, index) in dataReport" :key="index">
                  ...
                  <td>{{item.ec_name}}</td>
                  <td>{{ item.status_student }}</td>
                  <td v-if="['999999999',102].indexOf(role_id)!= -1">{{ item.gud_mobile1 }}</td>
                </tr>
              </tbody>
            </table>
          </div>
...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      ...
      var urlApi = "/api/export/report_r04"
      var tokenKey = u.token()
      u.g(urlApi, params, 1, 1)
        .then(response => {
          saveAs(response, "BÁO CÁO HỌC SINH MỚI.xlsx");
          this.processing = false;
        })
        .catch(e => {
          this.processing = false;
