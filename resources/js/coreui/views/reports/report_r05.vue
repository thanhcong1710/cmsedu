            ...
            >
              <thead>
                <tr class="text-sm">
                  ...
                  <th>Ngày thu</th>
                  <th>Mã khách hàng</th>
                  <th>Phân loại học viên (Cũ/mới)</th>
                  ...
                  <th>Trung tâm NVKD</th>
                  <th>Diễn giải</th>
                  <th>GT khóa học</th>
                  <th>Tiền giảm khác</th>
                  <th>Mã CC/CK</th>
                  <th>Số tiền giảm trừ theo mã CC/CK</th>
                  <th>Tiền sau CK</th>
                  <th>Đã nộp</th>
                  <th>Số tiền còn lại phải thu</th>
                  <th>Số chứng từ PNH</th>
                  <th>Doanh số bán chéo</th>
                  <th>Trung tâm  bán chéo</th>
                  <th>TVV Trung tâm bán chéo</th>
                  <th>Doanh số thực thu của  trung tâm</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(item, index) in dataReport" :key="index">
                  ...
                  <td>{{ item.count_recharge ==0 ? 'Mới' : 'Tái tục' }}</td>
                  <td>{{ item.name }}</td>
                  ...
                  <td>{{ item.ec_hrm }}</td>
                  <td>{{item.ec_name}}</td>
                  <td>{{ item.ec_branch }}</td>
                  <td>{{ item.note }}</td>
                  <td>{{ item.tuition_price }}</td>
                  ...
                  <td>{{ item.amount }}</td>
                  <td>{{ item.debt }}</td>
                  <td>{{ item.contract_accounting }}</td>
                  <td>{{ item.count_recharge ==0 && item.pre_branch ? item.amount/2 :'' }}</td>
                  <td>{{ item.count_recharge ==0 && item.pre_branch ? item.pre_branch :'' }}</td>
                  <td>{{ item.count_recharge ==0 && item.pre_branch ? item.pre_ec_name :'' }}</td>
                  <td>{{ item.count_recharge ==0 && item.pre_branch ? item.amount/2 : item.amount }}</td>
                </tr>
              </tbody>
              <tr>
                <td colspan="18"><b>Tổng</b></td>
                <td>{{summary.must_charge | formatMoney}}</td>
                <td>{{summary.amount | formatMoney}}</td>
                <td></td>
                <td></td>
                <td>{{summary.ban_cheo | formatMoney}}</td>
                <td></td>
                <td></td>
                <td>{{summary.thuc_thu | formatMoney}}</td>
              </tr>
            </table>
          </div>
...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      ...
      var urlApi = "/api/export/report_r05"
      var tokenKey = u.token()
      u.g(urlApi, params, 1, 1)
        .then(response => {
          saveAs(response, "BÁO CÁO DOANH THU.xlsx");
          this.processing = false;
        })
        .catch(e => {
          this.processing = false;
    ...
    getParamsSearch() {
      ...
      const from_date = this.searchData.dateRange!='' && this.searchData.dateRange[0] ?`${u.dateToString(this.searchData.dateRange[0])}`:'2022-06-01'
