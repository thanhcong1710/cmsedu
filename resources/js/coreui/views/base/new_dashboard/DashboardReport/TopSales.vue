	...
	<b-card header>
		...
		<div class="table-responsive scrollable fixed-table">
			<table class="table table-striped table-bordered apax-table">
				<thead>
					<tr class="text-sm">
						<th class="width-50">Mã Sale</th>
						<th class="width-50">Tên Sale</th>
						<th class="width-50">Doanh số</th>
						<th class="width-80">Trung tâm</th>
					</tr>
				</thead>
				<tbody>
					<tr v-for="(sa, index) in sale" :key="index">
						<td :class="{bgColor: check() && index == 0 , nobgColor: !check() && index==0}">{{sa.ma_nv}}</td>
						<td :class="{bgColor: check() && index == 0 , nobgColor: !check() && index==0}">{{sa.ten_nv}}</td>
						<td :class="{bgColor: check() && index == 0 , nobgColor: !check() && index==0}">{{sa.doanhso | formatMoney}}</td>
						<td :class="{bgColor: check() && index == 0 , nobgColor: !check() && index==0}">{{sa.ten_tt}}</td>
					</tr>
				</tbody>
			</table>
		</div>
