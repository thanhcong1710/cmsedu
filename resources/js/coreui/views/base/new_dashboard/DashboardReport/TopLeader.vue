	...
	<b-card header>
		...
		<div class="table-responsive scrollable fixed-table">
			...
			<table class="table table-striped table-bordered apax-table">
				<thead>
					<tr class="text-sm">
						<th class="width-50">Mã Leader</th>
						<th class="width-50">Tên Leader</th>
						<th class="width-50">Doanh số</th>
						<th class="width-80">Trung tâm</th>
					</tr>
				</thead>
				<tbody>
					<tr v-for="(le, index) in leader" :key="index">
						<td :class="{bgColor: check() && index == 0 , nobgColor: !check() && index==0}">{{le.ma_nv}}</td>
						<td :class="{bgColor: check() && index == 0 , nobgColor: !check() && index==0}">{{le.ten_nv}}</td>
						<td :class="{bgColor: check() && index == 0 , nobgColor: !check() && index==0}">{{le.doanhso | formatMoney}}</td>
						<td :class="{bgColor: check() && index == 0 , nobgColor: !check() && index==0}">{{le.ten_tt}}</td>
					</tr>
				</tbody>
			</table>
		</div>
