                            ...
                            <div class="table-responsive scrollable">
                                <table class="table table-striped table-bordered apax-table">
                                    ...
                                    <tr class="text-sm">
                                        ...
                                        <th>Lớp đi</th>
                                        <th>Lớp đến</th>
                                        <th>Ngày chuyển lớp</th>
                                        <!--<th>Trạng thái</th>-->
                                        <th>Thao tác</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    <tr v-for="(branch_transfer, index) in list" :key="index">
                                        <td><router-link v-b-tooltip.hover title="Nhấp vào để xem chi tiết" class="link-me" :to="`class-transfers/${branch_transfer.student_id}`">{{index+1}}</router-link></td>
                                        <td><router-link v-b-tooltip.hover title="Nhấp vào để xem chi tiết" :to="`class-transfers/${branch_transfer.student_id}`">{{branch_transfer.lms_id}}</router-link></td>
                                        <td><router-link v-b-tooltip.hover title="Nhấp vào để xem chi tiết" :to="`class-transfers/${branch_transfer.student_id}`">{{branch_transfer.accounting_id}}</router-link></td>
                                        <td><router-link v-b-tooltip.hover title="Nhấp vào để xem chi tiết" class="link-me" :to="`class-transfers/${branch_transfer.student_id}`">{{branch_transfer.student_name}}</router-link></td>
                                        <td><router-link v-b-tooltip.hover title="Nhấp vào để xem chi tiết" class="link-me" :to="`class-transfers/${branch_transfer.student_id}`">{{branch_transfer.class_name}}</router-link></td>
                                        <td><router-link v-b-tooltip.hover title="Nhấp vào để xem chi tiết" class="link-me" :to="`class-transfers/${branch_transfer.student_id}`">{{branch_transfer.to_class_name}}</router-link></td>
                                         <td><router-link v-b-tooltip.hover title="Nhấp vào để xem chi tiết" class="link-me" :to="`class-transfers/${branch_transfer.student_id}`">{{branch_transfer.transfer_date}}</router-link></td>
                                        <!--<td>-->
                                            <!--<router-link v-b-tooltip.hover title="Nhấp vào để xem chi tiết" class="link-me" :to="`class-transfers/${branch_transfer.student_id}`">-->
                                                <!--<span v-if="branch_transfer.status < 3" class="text-warning">Chờ duyệt</span>-->
                                                <!--<span v-else-if="branch_transfer.status == 3" class="text-success">Đã duyệt</span>-->
                                                <!--<span v-else class="text-danger">Từ chối</span>-->
                                            <!--</router-link>-->
                                        <!--</td>-->
                                        <td>
                                            <span class="apax-btn detail disabled">
                                              <router-link v-b-tooltip.hover title="Nhấp vào để xem chi tiết" class="link-me" :to="`class-transfers/${branch_transfer.student_id}`"><i class="fa fa-eye"></i></router-link>
                                            </span>
                                        </td>
                                    </tr>
                                    </tbody>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            let creatable_roles = [ 676767,u.r.cm, u.r.om, u.r.branch_ceo, u.r.region_ceo, u.r.founder, u.r.admin, u.r.super_administrator];
            if (creatable_roles.indexOf(role) != -1) {
        ...
        methods: {
            ...
            filterData(type = 0){
                if(this.flags.form_loading === false){
                    ...
                    u.a().get(`/api/class-transfers?filter=${data}&pagination=${pagination}`)
                        .then(response => {
