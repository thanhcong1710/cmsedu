                          ...
                          <table class="table table-striped table-bordered apax-table">
                            ...
                            <tbody>
                              <tr v-for="(student, ind) in cache.students" v-bind:key="ind">
                                ...
                                <td>{{ student.calculate_sessions}}</td>
                                <td>{{ student.last_date | formatDate}}</td>
                                <td><div v-html="studentActive(student.last_date)"></div>
                                    <div v-if="enableWithdraw(student.last_date)" v-b-tooltip.hover title="Click vào đây để withdraw học sinh này luôn!" class="withdraw-button apax-btn primary" @click="withdraw(student)"><i class="fa fa-close fa-lg"></i></div>
                                </td>
                                ...
                                <!-- <td>
                                    <span v-if="studentExtend(student.last_date,student.is_extend)" class="success-label apax-label" @click="processStudentExtend(student)">
                                        <i v-b-tooltip.hover title="Nhấp vào để nối phí học sinh này" class="fa fa-refresh"></i>
                                    </span>
                                </td> -->
                              </tr>
                            </tbody>
      ...
      >
          <div class="col-md-12">
              <div class="search-contracts buttons-bar">
                  ...
                  ><i class="fa fa-save"></i> Thêm học sinh vào lớp
                  </apaxButton>
              ...
              <div class="table-responsive scrollable">
                  ...
                  <table class="table table-striped table-bordered apax-table contracts-list">
                      ...
                      <tr v-for="(contract, ind) in cache.contracts" :key="ind">
                          <td align="center" :class="checkingClass(contract)">
                              <b-form-checkbox :disabled="checkIsAvailable(contract)" class="check-item" v-model="cache.checked_list" :value="contract.contract_id"  @change.native="checkItem(contract)" number></b-form-checkbox>
                          </td>
                          <td align="right" :class="checkingClass(contract)">{{ ind + 1 }}</td>
                          <td :class="checkingClass(contract)">{{ contract.student_name }}</td>
                          <td :class="checkingClass(contract)">{{ contract.student_cms_id }}</td>
                          <td :class="checkingClass(contract)">{{ contract.student_school }}</td>
                          <td :class="checkingClass(contract)">{{ contract.gud_name1 }}</td>
                          <td :class="checkingClass(contract)">{{ contract.gud_mobile1 }}</td>
                          <td :class="checkingClass(contract)">{{ contract.contract_start_date }}</td>
                          <td :class="checkingClass(contract)">
                              <select :blur="onBlurCheckBox(contract)"
                                      :disabled="checkIsAvailable(contract)"
                          ...
                          <td :class="checkingClass(contract)">{{ contract.customer_type | contractType }}</td>
                          <td :class="checkingClass(contract)" class="normal-font"><span v-html="showTuitionFee(contract)"></span></td>
                          <td :class="checkingClass(contract)">{{ contract.available_sessions }}</td>
                          <td :class="checkingClass(contract)">{{ contract.charged_total | formatMoney }}</td>
                          <td :class="checkingClass(contract)">{{ contract.ec_name }}</td>
                      </tr>
                      </tbody>
                  </table>
              </div>
...
export default {
  ...
  computed: {
    isAdministrator () {
      return parseInt(_.get(u.session(), 'user.role_id')) === 999999999
    },
    selectAll: {
      get: function () {
  ...
  methods: {
    ...
    link(semester = false) {
      ...
      if (semester) {
        ...
        const search = u.jss({
          ...
          semester: this.cache.semester,
          get_all_class_dates: this.isAdministrator ? 1: 0
        })
    ...
    availableButton() {
      let result = ''
      if ([36,55,56,57,58,676767,686868,7777777,999999999].indexOf(this.session.user.role_id) > -1) {
          result = ''
    ...
    studentActive(v) {
      var lastDate = new Date(v)
      var currentDate = new Date()

      if (lastDate > currentDate)
        return '<div class="success-label apax-label">Active</div>'
      else
        return '<div class="alert-label apax-label">Expired</div>'
    ...
    studentExtend(s, ext) {
      if (this.session.user.role_id == 999999999 && ext > 1)
        return true
      var lastDate = new Date(s)
      var currentDate = new Date()
    ...
    filterStartDate(student, class_dates) {
      ...
      // console.log('isAdministrator', this.isAdministrator, student, class_dates)
      const resp = []
    ...
    load(data, semester = false){
      if (semester) {
        ...
        // this.$root.$emit('bv::show::modal', 'add-contract-register')
    ...
    start() {
      if (u.authorized() || u.session().user.role_id == 84 || u.session().user.role_id == 56) {
    ...
    printForm(data) {
      // console.log(data)
      window.open(`/print/register/${data.contract_id}`,'_blank')
    },
    processStudentExtend(data) {
      const confirmation = confirm("Bạn có chắc là muốn nối phí cho học sinh này?")
      if (confirmation) {
        this.html.loading.action = true
        u.p('/api/enrolments/student-extend', data)
          .then(response => {
            this.html.loading.action = false
            this.selectOneClass(response)
    ...
    selectOneClass(class_id) {
      this.html.loading.action = true
      u.g(`${this.url.class}${class_id}`)
        .then(response => {
          const data = response
          this.cache.class_info = data.class
          this.cache.students = data.students
          if(data.class.class_end_date < this.getDate(new Date())){
          ...
          if(this.checkRole()){
            ...
            if (!data.class.teacher_id || data.class.teacher_id === null) {
              this.html.disable.load_contracts = true
            }
            if (parseInt(data.class.class_max_students, 10) === parseInt(data.class.total_students, 10)) {
              this.html.disable.load_contracts = true
            }
          }else{
    ...
    showTuitionFee(contract) {
      let resp = contract.tuition_fee_name !== null && contract.tuition_fee_price !== null ? `${contract.tuition_fee_name} (${contract.tuition_fee_price})` : 'Học sinh học trải nghiệm chương trình được miễn phí'
      return resp
    },
    checkItem(contract) {
      this.html.disable.save_contracts = false
    ...
    checkingClass(contract) {
      let resp = ''
      return resp
    },
    alertEffectNull(contract) {
      return (contract.student_accounting_id === null || contract.student_accounting_id == '') && contract.type > 0 ? 'Không thể xếp lớp khi chưa có mã Effect' : contract.student_accounting_id
    },
    addContracts() {
      if (this.cache.available < 0) {
        ...
        if (parseInt(this.cache.available) > 0 && parseInt(selected_contracts.length - parseInt(this.cache.available)) > 0) {
          ...
          selected_contracts.every(selected_contract => {
            if (!selected_contract.class_date || selected_contract.class_date.toString() === '') {
              alert(`Học sinh đã chọn ${selected_contract.student_name} chưa được chọn ngày bắt đầu học, vui lòng chọn ngày bắt đầu học cho học sinh ${selected_contract.student_name}`)
              checked = false
    ...
    checkRole(){
      const deniedRoles = [u.r.ec,u.r.ec_leader,83,84,100,7676767];
      const userRoleId = parseInt(this.session.user.role_id);
      const deniedPermission = deniedRoles.indexOf(userRoleId) > -1;
      if (deniedPermission) {
        this.html.disable.load_contracts = true
      }
    ...
    withdraw(student) {
      ...
      if (confirmation) {
          this.action.loading = true
        u.g(`/api/enrolments/withdraw/${student.contract_id}`)
          .then(response => {
            this.action.loading = false
            this.selectClass(this.cache.selected_class)
          }).catch(e => u.log('Exeption', e))
...
<style lang="scss">
  #add-contract-register .modal-dialog {
    width: 96% !important;
...
#add-contract-register___BV_modal_footer_ button:first-child {
