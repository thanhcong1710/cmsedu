                    ...
                    <div class="row">
                      <div class="col-md-6">
                        <div class="form-group">
                          ...
                          <input class="form-control" :value="student.birthday | formatDate" type="text" readonly>
                        </div>
                      </div>
                      <div class="col-md-6">
                        <div class="form-group">
                      ...
                      <div class="row">
                        ...
                        <div class="col-md-6">
                          <div class="form-group">
                            ...
                            <div v-else>
                              <input class="form-control" :value="contract.program_name" type="text" readonly>
                            </div>-->
                      ...
                      <div class="row">
                        <div class="col-md-12 pad-no" :class="html.dom.display.receive">
                          <div class="form-group">
                            ...
                            <div class="detail-bill-info-frame">
                              <div class="detail-bill-info" v-html="contract.bill_info"></div>
                            </div>
                          </div>
                        </div>
                      </div>
                          ...
                          <div class="form-group">
                            ...
                            <div v-if="first === contract.id && contract.editable_start_date === 1">
                              <calendar
                                ...
                                :not-before="contract.start_date"
                      ...
                      <div class="row">
                        <div class="col-md-6">
                          <div class="form-group"><input type="hidden" v-model="data.current_cm" />
                            <label class="control-label">CM <button :disabled="html.dom.disabled.update_cm" :class="html.dom.display.update_cm" class="button-save-cm" title="Cập nhật CM" value="Lưu" @click="html.dom.action.update_cm" >Lưu</button></label>
                            <select v-model="cm" :disabled="html.dom.disabled.select_cm" id="select_cm" class="selection cm form-control" @change="html.dom.action.select_cm">
...
export default {
  ...
  data () {
    ...
    model.html.dom = {
      ...
      quit: {
        ...
        title: 'Bỏ cọc, hủy hợp đồng tái phí này',
        markup: 'primary',
        disabled: false,
      ...
      back: {
        label: 'Thoát',
        icon: 'fa-sign-out',
        title: 'Quay lại danh sách tái tục',
        markup: 'error',
        disabled: false,
        action: () => this.exitForm()
      },
      print: {
        label: 'In bản ghi tái phí',
        icon: 'fa-print',
        title: 'In nội dung bản ghi tái phí đang xem',
        markup: 'success',
        disabled: false,
  ...
  methods: {
      ...
      .then(response => {        
        ...
        if (response.relation_info && response.relation_info.latest_contract) {
          ...
          if (this.latest_contract && this.latest_contract.enrolment_schedule) {
              this.class_days = this.latest_contract.enrolment_schedule.toString().split(',')
        ...
        if((this.contract.type == 3) || (this.contract.type == 4)){
            this.html.dom.print.disabled = true;
        }else{
            this.html.dom.print.disabled = false;
        }
        if (this.contract.only_give_tuition_fee_transfer === 0) {
    ...
    onDrawStartDate(e) {
        let date = e.date
        if (this.moment(this.contract.start_date) > this.moment(date.getTime())) {
            e.allowSelect = false
        }
    },
    quitContract() {
      ...
      .then(response => {        
        if (response.ok) {
          u.apax.$emit('apaxPopup', {
            on: true,
            content: `Hồ sơ tái phí mã: ${response.code}<br/>Đã được hủy thành công!`,
            title: 'Hồ Sơ Tái Phí Đã Bị Hủy Do Bỏ Cọc',
            class: 'modal-danger',
            size: 'md',
            variant: 'apax-ok'
          })
    ...
    updateEc() {
      ...
      .then(response => {
        this.curec = this.ec
        this.contract.contract_ec_leader_name = response.ec_leader_name
        this.html.dom.display.update_ec = 'hidden'
    ...
    updateCm() {
      ...
      .then(response => {
        this.curcm = this.cm
        this.contract.contract_om_name = response.om_name
        this.html.dom.display.update_cm = 'hidden'
    ...
    checkQuit(contract) {
      let resp = false
      if (contract.status == 7 && parseInt(contract.status, 10) > 0 && parseInt(contract.type, 10) > 0 && parseInt(contract.debt_amount, 10) >= 0) {
        resp = true
      }
...
.button-save-ec, .button-save-cm, .button-save-program {
...
.button-save-ec:hover, .button-save-cm:hover, .button-save-program:hover {
