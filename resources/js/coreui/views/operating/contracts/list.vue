  ...
  <div class="animated fadeIn apax-form" @keyup="binding" id="contracts-management">
      ...
      <div class="col-12">
        <b-card header>
                ...
                <div class="panel">
                  <div class="panel-body">
                    <div class="row">
                      ...
                      <div class="col-3" v-if="moreOptionFilter">
                        <div class="form-group">
                          <label class="filter-label control-label">Tạo từ ngày</label><br>
                          <datepicker
                            v-model="html.data.filter.created_start_date"
                            :readonly="false"
                            lang="en"
                      ...
                      <div class="col-3" v-if="moreOptionFilter">
                        <label class="filter-label control-label">Tạo đến ngày</label><br>
                        <datepicker
                          v-model="html.data.filter.created_end_date"
                          lang="en"
                          input-class="form-control bg-white"
                          placeholder="Chọn ngày"
                          :bootstrap-styling="true"
          ...
          <div slot="footer" class="text-center">
            <router-link to="/contracts/add" v-show="onlyView()">
              <button type="button" v-b-tooltip.hover title="Thêm bản ghi nhập học mới" class="apax-btn full reset">
                <i class="fa fa-plus"></i> Thêm mới
              </button>
            </router-link>
            <button @click="listContract(0)" v-b-tooltip.hover title="Xem danh sách nhập học" class="apax-btn full edit"><i class="fa fa-list"></i> {{html.dom.filter.buttons.default}}</button>
            <button @click="listContract(1)" v-b-tooltip.hover title="Xem danh sách chưa đóng phí" class="apax-btn full detail"><i class="fa fa-slack"></i> {{html.dom.filter.buttons.waitcharged}}</button>
            <button @click="listContract(2)" v-b-tooltip.hover title="Xem danh sách đã đóng phí" class="apax-btn full print"><i class="fa fa-angellist"></i> {{html.dom.filter.buttons.charged}}</button>
            <button @click="listContract(3)" v-b-tooltip.hover title="Xem danh sách chờ xếp lớp" class="apax-btn full remove"><i class="fa fa-send-o"></i> {{html.dom.filter.buttons.waiting}}</button>
            <button @click="() => {moreOptionFilter = !moreOptionFilter}" v-b-tooltip.hover title="Thêm tùy chọn lọc" class="apax-btn full reset"><i class="fa fa-eye" /> {{ moreOptionFilter ? "Bỏ bớt tùy chọn lọc": "Thêm tùy trọn lọc" }}</button>
          </div>
        </b-card>
      </div>
    </div>
    <div class="row drag-me-up">
      <div class="col-12">
        ...
        <b-card header>
          <div slot="header">
            <i class="fa fa-list"></i> <b class="uppercase">Danh sách đăng ký nhập học</b>
          </div>
              ...
              <div class="table-responsive scrollable">
                ...
                <tbody>
                  <tr v-for="(item, index) in list" :key="index">
                      ...
                      <td><router-link v-b-tooltip.hover class="link-me" :title="`${html.page.title}`" :to="`${html.page.url.link}${item.id}`">{{item.must_charge | formatMoney}}</router-link></td>
                      ...
                      <td>
                        <!-- <span class="apax-btn edit" v-if="item.debt_amount > 0 || session.user.role_id == '999999999'">
                        ...
                        <span class="apax-btn print" @click="print(item)" v-if="item.contract_type != 4">
      ...
      <div class="printing-area hidden">
        <contractPrint :print="html.data.print.the_contract" :infor="html.data.print.contract"></contractPrint>
        <trailPrint :print="html.data.print.the_registration" :infor="html.data.print.trail"></trailPrint>
      </div>
    </div>
  </div>
</template>
...
import contractPrint from '../../base/prints/contract'
import trailPrint from '../../base/prints/trail_register'
...
export default {
  ...
  data() {
    const model = u.m('contracts').list
    model.html.dom = {
      filter: {
        buttons: {
          default: 'Danh sách nhập học',
  ...
  methods: {
    ...
    link (link = '') {
      ...
      const created_start_date     = getDate(this.html.data.filter.created_start_date)
      const created_end_date       = getDate(this.html.data.filter.created_end_date)
      this.html.data.filter.keyword = this.html.data.filter.keyword.trim()
      console.log(this.html.data.filter.keyword)
      const filter                 = Object.assign({}, this.html.data.filter, { created_start_date, created_end_date })
      const sort                   = u.jss(this.html.order)
      const search                 = u.jss(filter)
    ...
    print(contract) {
      if (contract.contract_type === 0) {
        window.open(`/print/trial-register/${contract.id}`,'_blank')
