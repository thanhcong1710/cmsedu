    ...
    <div class="animated fadeIn apax-form">
        <div class="row">
            <div class="col-12">
                <b-card header>
                    <div slot="header">
                        <i class="fa fa-id-card" /> <b class="uppercase">Cập nhật cộng tác viên</b>
  ...
  export default {
    ...
    methods: {
      validateForm () {
        ...
        const { personal_name, code, address, phone_number, school_name } = form
        // eslint-disable-next-line camelcase
        return (code && school_name) || false
      },
      handleGoBack () {
        this.$router.go(-1)
      },
        ...
        u.p(url, this.form)
          .then((response) => {
            if (response.error_code === 0)
              return this.$router.push('/collaborator')
            else
      ...
      validate (params) {
        let message = ''
        let status  = true
        if (_.get(params, 'phone_number') && isNaN(_.get(params, 'phone_number'))) {
      ...
      getDiscountCode (id) {
        const uri = `/api/collaborator-detail1/${id}`
